# Souk — Store Listing Copy

Copy for Google Play Console and App Store Connect submissions. Character
limits are noted per field; all three launch languages (EN / AR / FR) are
covered. Bundle ID: `com.souklb.app`.

---

## App name / title (Play: 30 chars · App Store: 30 chars)

| Lang | Title |
|---|---|
| EN | Souk — Local Boutique Market |
| AR | سوق — بوتيكات لبنان |
| FR | Souk — Boutiques du Liban |

## App Store subtitle (30 chars)

| Lang | Subtitle |
|---|---|
| EN | Shop Lebanon's boutiques |
| AR | تسوّق من بوتيكات لبنان |
| FR | Achetez local au Liban |

## Short description (Play Store, 80 chars)

| Lang | Short description |
|---|---|
| EN | Shop new & thrifted fashion from local Lebanese boutiques, delivered to you. |
| AR | تسوّق أزياء جديدة ومستعملة من بوتيكات لبنانية محلية، توصَل إليك. |
| FR | Mode neuve et seconde main des boutiques libanaises, livrée chez vous. |

## Full description (Play: 4000 chars · App Store: 4000 chars)

### EN

**Souk brings Lebanon's boutiques to your pocket.**

Discover new and thrifted fashion from independent local sellers — browse,
order, and track delivery to your door, all in one app.

**For shoppers**
• Browse a curated feed of local boutiques — new arrivals, thrifted picks, and featured stores
• Search and filter by category, region, and price
• See sizes, colours, and real stock before you order
• Check out your way: WhatsApp, cash on delivery, or bank transfer — no card required
• Track every order from confirmation to your doorstep, with courier updates
• Earn loyalty tiers and discount vouchers as you shop; refer friends and you both get rewarded
• Read real reviews from verified buyers
• Save favourites and share finds with friends
• Use Souk in English, Arabic, or French

**For sellers**
• Open your store in minutes — no fees to join
• List products with photos, variants, and stock counts
• Manage orders through a clear pipeline: accept, prepare, dispatch, deliver
• Assign couriers and give buyers live tracking
• Get instant WhatsApp notifications for new orders
• See your revenue, top categories, and regional reach at a glance

Payments settle directly between you and the seller — the way business is
actually done in Lebanon. No cards, no friction, no waiting.

Souk is free to join for shoppers and sellers.

### AR

**سوق يضع بوتيكات لبنان في جيبك.**

اكتشف أزياء جديدة ومستعملة من بائعين محليين مستقلين — تصفّح، اطلب، وتتبّع التوصيل حتى باب بيتك، كل ذلك في تطبيق واحد.

**للمتسوقين**
• تصفّح بوتيكات محلية مختارة — وصولات جديدة، مختارات مستعملة، ومتاجر مميزة
• ابحث وصفِّ حسب الفئة والمنطقة والسعر
• شاهد المقاسات والألوان والكمية الفعلية قبل الطلب
• ادفع على طريقتك: واتساب، نقدًا عند الاستلام، أو تحويل مصرفي — لا حاجة لبطاقة
• تتبّع كل طلب من التأكيد حتى باب بيتك مع تحديثات التوصيل
• اكسب مستويات ولاء وقسائم خصم مع كل طلب؛ أحِل أصدقاءك واربحا معًا
• اقرأ تقييمات حقيقية من مشترين موثّقين
• احفظ المفضلة وشارك اكتشافاتك مع أصدقائك
• استخدم سوق بالعربية أو الإنجليزية أو الفرنسية

**للبائعين**
• افتح متجرك في دقائق — الانضمام مجاني
• اعرض منتجاتك مع الصور والخيارات والكميات
• أدر طلباتك بمسار واضح: قبول، تحضير، إرسال، توصيل
• عيّن موصّلين وامنح المشترين تتبعًا مباشرًا
• استقبل إشعارات واتساب فورية للطلبات الجديدة
• شاهد إيراداتك وأفضل فئاتك وانتشارك الجغرافي بنظرة واحدة

الدفع يتم مباشرة بينك وبين البائع — كما تُدار الأعمال فعلًا في لبنان. بلا بطاقات، بلا تعقيد، بلا انتظار.

الانضمام إلى سوق مجاني للمتسوقين والبائعين.

### FR

**Souk met les boutiques du Liban dans votre poche.**

Découvrez la mode neuve et seconde main de vendeurs locaux indépendants —
parcourez, commandez et suivez la livraison jusqu'à votre porte, le tout dans
une seule app.

**Pour les acheteurs**
• Parcourez un fil de boutiques locales sélectionnées — nouveautés, seconde main et boutiques à la une
• Recherchez et filtrez par catégorie, région et prix
• Consultez tailles, couleurs et stock réel avant de commander
• Payez à votre façon : WhatsApp, espèces à la livraison ou virement bancaire — sans carte
• Suivez chaque commande de la confirmation à votre porte, avec les mises à jour du coursier
• Gagnez des niveaux de fidélité et des bons de réduction ; parrainez vos amis et soyez récompensés tous les deux
• Lisez de vrais avis d'acheteurs vérifiés
• Enregistrez vos favoris et partagez vos trouvailles
• Utilisez Souk en français, arabe ou anglais

**Pour les vendeurs**
• Ouvrez votre boutique en quelques minutes — inscription gratuite
• Publiez vos produits avec photos, variantes et stocks
• Gérez vos commandes avec un pipeline clair : accepter, préparer, expédier, livrer
• Assignez des coursiers et offrez un suivi en direct
• Recevez des notifications WhatsApp instantanées pour chaque commande
• Visualisez revenus, meilleures catégories et portée régionale en un coup d'œil

Les paiements se règlent directement entre vous et le vendeur — comme les
affaires se font réellement au Liban. Sans carte, sans friction.

Souk est gratuit pour les acheteurs comme pour les vendeurs.

## Keywords (App Store, 100 chars, comma-separated, EN)

```
boutique,thrift,fashion,lebanon,beirut,marketplace,local,shopping,delivery,vintage,souk
```

## Category

- Primary: Shopping
- Secondary: Lifestyle

## Content rating notes

- No user-generated content beyond product listings and order reviews (moderated by store owners / platform)
- No gambling, no violence; commerce app
- Collects: email (auth), phone (optional, order updates), delivery address (orders) — declare in Play Data Safety and App Store Privacy Nutrition labels

## Support / marketing URLs

- Web app: https://souk-app.expo.app
- Support email: hfakih1212@gmail.com (replace with a support@ alias before launch)

---

## Screenshots — still needed

Currently in `screenshots/`: `login-desktop.png`, `login-mobile.png` (not
store-ready — wrong framing, login screen only).

Needed per store (portrait phone, device-framed or clean full-bleed):

1. Marketplace feed with editorial rails (hero shot)
2. Product detail with variants + Add to Bag
3. Cart → checkout with payment methods
4. Order tracking timeline
5. Vendor dashboard with analytics strip
6. Product management / add product

Capture at 1080×1920+ from the production web app (https://souk-app.expo.app
in a phone-sized viewport) or an Android emulator running the dev build.
Play Store also wants a 1024×500 feature graphic; App Store wants 6.7" and
6.5" sets (can be generated from one 1290×2796 capture set).
