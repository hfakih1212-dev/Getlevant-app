...
Deno.serve(async (req: Request) => {
  ...
  const FROM_EMAIL = Deno.env.get('RESEND_FROM_EMAIL') ?? 'Getlevant <onboarding@resend.dev>'
...
<body style="font-family:sans-serif;background:#FAF7F2;margin:0;padding:40px 20px">
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:40px;box-shadow:0 2px 8px rgba(0,0,0,.08)">
    <h1 style="color:#1C1612;font-size:24px;margin:0 0 8px">Your Getlevant sign-in code</h1>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: [user.email],
        subject: `${otp} is your Getlevant sign-in code`,
        html,
      }),
    })
