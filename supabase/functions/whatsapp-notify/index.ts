...
function buildNewOrderMessage(order: Record<string, unknown>): string {
  ...
  return [
    ...
    '',
    'Open the Souk app to accept this order.',
...
function buildStatusMessage(order: Record<string, unknown>, newStatus: string): string | null {
  ...
  if (newStatus === 'dispatched') {
    ...
    const lines = [`🚚 Your Souk order *${num}* is on its way!`]
    ...
    lines.push('', 'Track your delivery in the Souk app.')
    return lines.join('\n')
  }

  const STATIC: Partial<Record<string, string>> = {
    confirmed: `✅ Your Souk order *${num}* is confirmed! The vendor is getting it ready.`,
    preparing: `⏳ Your Souk order *${num}* is being prepared.`,
    ready:     `📦 Your Souk order *${num}* is packed and ready for the courier.`,
    delivered: `🎉 Your Souk order *${num}* has been delivered! Enjoy your purchase.`,
    cancelled: `❌ Your Souk order *${num}* has been cancelled. Questions? Contact the vendor directly.`,
  }
